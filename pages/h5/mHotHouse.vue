<template>
  <div>
    <m-house-list title="热销房推荐" :houseType="3" :houseData="houseData"></m-house-list>
...
export default {
  ...
  async asyncData ({ $axios, deviceType, redirect, store, query }) {
    if (deviceType && deviceType.type == 'pc') {
      redirect(`/xcqHouse/houseRecommend/hot`)
    }

    let res = await $axios.post(`/api/index.php?ctl=fuhuaqi&type=3`);
  ...
  methods: {
    getXcqHouseList(type) {
      const vm = this
      this.$axios
        .post(`/api/index.php?ctl=fuhuaqi&type=3`, {})
        .then(res => {
          if(res.status === 200){
