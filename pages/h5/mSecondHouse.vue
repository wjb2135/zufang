<template>
  <div>
    <m-house-list title="精品二手房源" :houseType="2" :houseData="houseData"></m-house-list>
...
export default {
  ...
  async asyncData ({ $axios, deviceType, redirect, store }) {
    if (deviceType && deviceType.type == 'pc') {
      redirect('/xcqHouse/houseRecommend/second')
    }
    let res = await $axios.post(`/api/index.php?ctl=fuhuaqi&type=2`);
  ...
  methods: {
    getXcqHouseList(type) {
      const vm = this
      this.$axios
        .post(`/api/index.php?ctl=fuhuaqi&type=2`, {})
        .then(res => {
          let house_list = res.house_list
          if (house_list && Array.isArray(house_list)) {
            house_list.map(item => {
          ...
          EventBus.$emit('refreshCateType', {cateType: res.type})
        })
        .catch(err => {
          console.log(err)
        }) 
